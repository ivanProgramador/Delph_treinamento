...
{
Na sessão var são declaradas as variáveis que serão usadas durante a execução do codigo
as variveis delph podem ser:

 Integer(inteiros),
 Real(numeros fracionados)
...
{aqui começa oque vai contecer quando eu clicar}
begin

   {
     todos os valores necessaris vão vir atraves dos campo de texto
     então eu preciso converter o texto que vem deles para que possa afzer um
     calculo entre os dois
  }
  ...
  {
    O resultado recebeu a soma de dois numeros inteiros
    para mostrar essses numeros eu preciso fazer a conversão
    novamente para string
    porque a caixa de texto só aceita texto no retorno
  }
...
{aqui termina oque vai acontecer quando eu clicar}
...
procedure TForm1.btnDividirClick(Sender: TObject);
     ...
     Em caso de divisão é importante colocar as vearivbeis como real porque divisões podem rtronar valores quebrados
